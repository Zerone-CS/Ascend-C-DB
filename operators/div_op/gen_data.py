...
def generate_test_data():
    ...
    y = np.random.uniform(0.5, 2.0, total_length).astype(np.float32)
    y = y * np.random.choice([-1, 1], total_length).astype(np.float32)
...
def verify_result():
    ...
    if np.allclose(golden, result, rtol=1e-3, atol=1e-5):
        ...
        mismatch = np.where(~np.isclose(golden, result, rtol=1e-3, atol=1e-5))[0]
        print(f"Mismatch count: {len(mismatch)} / {len(golden)}")
