...
#include "SigmoidCustom/build_out/autogen/aclnn_sigmoid_custom.h"
...
int main() {
    ...
    ret = aclnnSigmoidCustomGetWorkspaceSize(x, out, &workspaceSize, &executor);
    ...
    LOG_PRINT("\n=== Sigmoid Custom Operator Test ===\n");
    LOG_PRINT("Input (first 10): ");
    for (int i = 0; i < 10; i++) {
        LOG_PRINT("%.2f ", xHostData[i]);
    }
    LOG_PRINT("\nNPU Output:       ");
    for (int i = 0; i < 10; i++) {
        LOG_PRINT("%.4f ", outHostData[i]);
    }
    LOG_PRINT("\nCPU Expected:     ");
    for (int i = 0; i < 10; i++) {
        LOG_PRINT("%.4f ", sigmoid_cpu(xHostData[i]));
    }
    LOG_PRINT("\n");
    ...
    int errorCount = 0;
    for (int i = 0; i < totalSize; i++) {
        float expected = sigmoid_cpu(xHostData[i]);
